...
const VoteFeedScreen: React.FC<ScreenProps> = ({ state }) => {
    ...
    <header className="bg-[#FFFAF5] px-5 pt-6 pb-2 sticky top-0 z-30">
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-2">
          ...
          <span className="font-['Nunito'] font-bold text-xl text-[#2C2C2C]">direkte</span>
        </div>
      ...
      <div className="bg-[#E5E7EB] p-1 rounded-xl flex mb-2">
        <button className={`flex-1 py-2.5 rounded-lg text-sm font-bold transition-all duration-200 ${state === 'closingSoon' ? 'text-[#6B7280] hover:text-[#2C2C2C]' : 'bg-white text-[#2C2C2C] shadow-sm'}`}>
          Current Votes
        </button>
        <button className={`flex-1 py-2.5 rounded-lg text-sm font-bold transition-all duration-200 ${state === 'closingSoon' ? 'bg-white text-[#2C2C2C] shadow-sm' : 'text-[#6B7280] hover:text-[#2C2C2C]'}`}>
          Closing Soon
        </button>
      </div>
    </header>
  ...
  const renderProposalCard = (title: string, summary: string, days: number, forPct: number, isUrgent: boolean = false) => (
      ...
      <div className="flex justify-between items-start mb-3">
        ...
        {isUrgent && (
          <div className="flex items-center gap-1 bg-[#FF6B35]/10 px-2 py-1 rounded-full">
            ...
            <span className="text-[10px] font-bold text-[#FF6B35] uppercase tracking-wide">{days}d</span>
          </div>
        )}
      </div>
      ...
      <div className="flex items-center justify-between mb-4">
        <div className="flex-1 mr-5">
          <div className="h-2.5 bg-[#F3F4F6] rounded-full overflow-hidden flex">
            <div className="bg-[#22A06B] h-full rounded-l-full" style={{ width: `${forPct}%` }}></div>
            <div className="bg-[#E34935] h-full rounded-r-full" style={{ width: `${100 - forPct}%` }}></div>
          </div>
          <div className="flex justify-between mt-2">
            <span className="text-[11px] font-bold text-[#22A06B]">For {forPct}%</span>
            <span className="text-[11px] font-bold text-[#E34935]">Against {100 - forPct}%</span>
          </div>
        </div>
  ...
  const renderEmptyState = () => (
    <div className="flex flex-col items-center justify-center pt-10 pb-6 px-6">
      ...
      <h2 className="font-['Nunito'] font-bold text-xl text-[#2C2C2C] mb-2 text-center">All Caught Up!</h2>
      <p className="text-center text-[#6B7280] text-sm mb-6">
        There are no active proposals open for voting right now. Check back soon or explore past results.
      </p>
    </div>
  );
  ...
  const renderHowItWorksCard = () => (
    <div className="mx-5 mb-20 bg-gradient-to-br from-[#5B4FCF] to-[#3D329F] rounded-2xl p-6 shadow-lg relative overflow-hidden">
      ...
      <div className="flex items-center gap-2 mb-4 relative z-10">
        ...
        <h3 className="font-['Nunito'] font-bold text-lg text-white">How It Works</h3>
      </div>
      
      <div className="space-y-4 relative z-10">
        <div className="flex gap-3">
          <div className="flex-shrink-0 w-6 h-6 rounded-full bg-white/20 flex items-center justify-center text-white text-xs font-bold font-['Space_Grotesk']">1</div>
          <p className="text-sm text-white/90 leading-relaxed">Read AI summaries of complex laws.</p>
        </div>
        <div className="flex gap-3">
          <div className="flex-shrink-0 w-6 h-6 rounded-full bg-white/20 flex items-center justify-center text-white text-xs font-bold font-['Space_Grotesk']">2</div>
          <p className="text-sm text-white/90 leading-relaxed">Cast your predictive vote anonymously.</p>
        </div>
        <div className="flex gap-3">
          <div className="flex-shrink-0 w-6 h-6 rounded-full bg-white/20 flex items-center justify-center text-white text-xs font-bold font-['Space_Grotesk']">3</div>
          <p className="text-sm text-white/90 leading-relaxed">Compare your voice with Parliament.</p>
        </div>
      </div>
