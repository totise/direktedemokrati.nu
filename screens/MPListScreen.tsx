  ...
  const renderHeader = () => (
    <header className="bg-[#FFFAF5] px-5 pt-6 pb-2 sticky top-0 z-30">
      ...
      <div className="relative mb-4">
        <Icons.Search
          className={`absolute left-4 top-1/2 transform -translate-y-1/2 w-5 h-5 transition-colors ${state === "searchActive" ? "text-[#5B4FCF]" : "text-[#9CA3AF]"}`}
        />
        <input
          type="text"
      ...
      <div className="flex gap-2 overflow-x-auto pb-2 no-scrollbar pr-2">
        <button
          className={`whitespace-nowrap px-4 py-1.5 rounded-full text-xs font-bold border-2 transition-all ${state === "partyFiltered" ? "bg-[#F3F4F6] border-[#F3F4F6] text-[#6B7280]" : "bg-[#5B4FCF] border-[#5B4FCF] text-white shadow-md shadow-[#5B4FCF]/20"}`}
        >
          All Parties
        </button>
        <button
          className={`whitespace-nowrap px-4 py-1.5 rounded-full text-xs font-bold border-2 transition-all bg-transparent border-[#E5E7EB] text-[#2C2C2C] hover:bg-[#F3F4F6]`}
        >
          Socialdem.
        </button>
        <button
          className={`whitespace-nowrap px-4 py-1.5 rounded-full text-xs font-bold border-2 transition-all ${state === "partyFiltered" ? "bg-[#00205B] border-[#00205B] text-white shadow-md" : "bg-transparent border-[#E5E7EB] text-[#2C2C2C] hover:bg-[#F3F4F6]"}`}
        >
          Venstre
        </button>
        <button
  ...
  const renderMPCard = (
    ...
    <div className="bg-[#FFFFFF] rounded-2xl p-4 shadow-[0_4px_16px_rgba(0,0,0,0.04)] mb-3 flex items-center gap-4 active:scale-[0.99] transition-transform duration-200">
      ...
      <div className="flex-1">
        <h3 className="font-['Nunito'] font-bold text-[#2C2C2C] text-lg leading-tight mb-1">
          {name}
        </h3>
