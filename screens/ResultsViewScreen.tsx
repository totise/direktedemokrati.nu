...
const ResultsViewScreen: React.FC<ScreenProps> = ({ state }) => {
    ...
    <header className="sticky top-0 z-30 bg-[#FFFAF5]/95 backdrop-blur-sm px-5 py-4 border-b border-[#E5E7EB]">
      ...
      <div className="relative mb-4">
        ...
        <input
          ...
          style={{
            borderColor: state === "searchActive" ? "#5B4FCF" : undefined
          }}
        />
        {state === "searchActive" && (
      ...
      <div className="flex gap-2 overflow-x-auto pb-1 pr-2 no-scrollbar">
        <button
          className={`whitespace-nowrap px-4 py-1.5 rounded-full text-xs font-bold transition-all ${isAllPartiesSelected ? "bg-[#5B4FCF] text-white shadow-md shadow-[#5B4FCF]/20" : "bg-[#F3F4F6] text-[#2C2C2C] hover:bg-[#E5E7EB]"}`}
        >
          All Parties
        </button>
        <button
          className={`whitespace-nowrap px-4 py-1.5 rounded-full text-xs font-bold transition-all bg-[#F3F4F6] text-[#2C2C2C] hover:bg-[#E5E7EB]'`}
        >
          Socialdem.
        </button>
        <button
          className={`whitespace-nowrap px-4 py-1.5 rounded-full text-xs font-bold transition-all ${isVenstreSelected ? "bg-[#00205B] text-white shadow-md" : "bg-[#F3F4F6] text-[#2C2C2C] hover:bg-[#E5E7EB]"}`}
        >
          Venstre
        </button>
        <button
  ...
  const renderComparisonHeader = () => (
    <div className="px-5 py-6 bg-white mb-2 shadow-sm">
      <h2 className="font-['Nunito'] font-bold text-sm text-[#6B7280] uppercase tracking-wide text-center mb-4">
        Final Vote Outcome
      </h2>
      ...
      <div className="flex items-center justify-center gap-6">
        ...
        <div className="flex flex-col items-center">
          <div className="w-24 h-24 rounded-full border-4 border-[#5B4FCF] flex flex-col items-center justify-center bg-[#5B4FCF]/5 relative">
            <span className="font-['Space_Grotesk'] font-bold text-2xl text-[#5B4FCF]">
              64%
            </span>
            <span className="text-[9px] uppercase font-bold text-[#5B4FCF]">
              For
            </span>
        ...
        <div className="flex flex-col items-center">
          <div className="w-24 h-24 rounded-full border-4 border-[#22A06B] flex flex-col items-center justify-center bg-[#22A06B]/5 relative">
            <span className="font-['Space_Grotesk'] font-bold text-2xl text-[#22A06B]">
              72%
            </span>
            <span className="text-[9px] uppercase font-bold text-[#22A06B]">
              For
            </span>
      ...
      <div className="mt-4 text-center">
        <p className="text-xs text-[#6B7280]">
          <span className="font-bold text-[#2C2C2C]">12,403</span> citizens
          voted
        </p>
      </div>
    </div>
  ...
  const renderMPAvatar = (mp: (typeof mps)[0], isDimmed: boolean) => {
    ...
    return (
      <div
        className={`flex flex-col items-center p-2 rounded-xl transition-all duration-200 ${isDimmed ? "opacity-30" : "hover:bg-[#F3F4F6]"}`}
      >
        <div className="relative mb-1">
          <div
            className="w-10 h-10 rounded-full p-0.5 border-2"
            style={{ borderColor: mp.partyColor }}
          >
            <div className="w-full h-full bg-[#F3F4F6] rounded-full overflow-hidden flex items-center justify-center">
              <span className="font-['Nunito'] font-bold text-xs text-[#6B7280]">
                {mp.name.charAt(0)}
              </span>
            </div>
          </div>
          ...
          {mp.vote === "for" && (
            <div className="absolute -bottom-1 -right-1 bg-[#22A06B] text-white rounded-full p-0.5 border-2 border-[#FFFAF5]">
              <Icons.Check className="w-2.5 h-2.5" />
            </div>
          )}
          {mp.vote === "against" && (
            <div className="absolute -bottom-1 -right-1 bg-[#E34935] text-white rounded-full p-0.5 border-2 border-[#FFFAF5]">
              <Icons.X className="w-2.5 h-2.5" />
            </div>
          )}
        </div>
        <span className="text-[9px] font-bold text-[#2C2C2C] text-center leading-tight w-12 truncate">
          {mp.name}
        </span>
      </div>
    );
  ...
  const renderMPGrid = () => {
    ...
    return (
      <div className="px-5 pb-10">
        <h3 className="font-['Nunito'] font-bold text-sm text-[#2C2C2C] mb-3">
          Individual Votes
        </h3>
        <div className="flex gap-2">
          ...
          <div className="flex-1 flex flex-col gap-1">
            <div className="bg-[#E34935]/10 rounded-lg p-2 text-center mb-1">
              <span className="text-xs font-bold text-[#E34935]">
                Against ({againstMPs.length})
              </span>
            </div>
            {againstMPs.map((mp) => (
              <React.Fragment key={mp.id}>
