...
const UserVotesHistoryScreen: React.FC<ScreenProps> = ({ state }) => {
  ...
  const renderVoteCard = (
    ...
    const userWon =
      status !== "pending" &&
      ((userVote === "for" && status === "passed") ||
        (userVote === "against" && status === "rejected"));

    return (
      <div className="bg-[#FFFFFF] rounded-2xl p-4 mb-3 shadow-[0_4px_16px_rgba(0,0,0,0.04)] flex items-center gap-4">
        {/* User Vote Indicator */}
        <div
          className={`flex flex-col items-center justify-center w-12 h-12 rounded-xl ${userVote === "for" ? "bg-[#22A06B]/10" : "bg-[#E34935]/10"}`}
        >
          {userVote === "for" ? (
            <Icons.ThumbsUp className="w-6 h-6 text-[#22A06B]" />
          ) : (
            <Icons.ThumbsDown className="w-6 h-6 text-[#E34935]" />
          )}
          <span className="text-[9px] font-bold uppercase mt-1 text-[#6B7280]">
            {userVote}
          </span>
        </div>
        ...
        <div className="flex-1">
          <h3 className="font-['Nunito'] font-bold text-base text-[#2C2C2C] leading-snug mb-1">
            {title}
          </h3>
          <div className="flex items-center gap-2 text-xs">
            <span className="text-[#6B7280] font-['Space_Grotesk']">
              {date}
            </span>
            {status !== "pending" && (
              <>
                ...
                <span
                  className={`font-bold ${status === "passed" ? "text-[#22A06B]" : "text-[#E34935]"}`}
                >
        ...
        {status !== "pending" && (
          <div
            className={`px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-wide ${userWon ? "bg-[#22A06B] text-white" : "bg-[#E34935] text-white"}`}
          >
  ...
  const renderEmptyState = () => (
    <div className="flex flex-col items-center justify-center pt-16 px-6">
      ...
      <p className="text-center text-[#6B7280] text-sm mb-8">
        You haven't cast any predictive votes. Go to the Vote feed to make your
        voice heard on current proposals.
      </p>
